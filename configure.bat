...
@set OGRE_INCLUDE_DIR=%OGRE_PATH%\include
@set OGRE_LIBRARY_DIR=%OGRE_PATH%\lib\%build_type%
@set OGRE_PLUGIN_DIR=%OGRE_LIBRARY_DIR%\opt
@set OGRE_RTSHADERSYSTEM_LIBS=%OGRE_LIBRARY_DIR%\OgreRTShaderSystem.lib
@set OGRE_LIBS=%OGRE_LIBRARY_DIR%\OgreMain.lib
@set OGRE_TERRAIN_LIBS=%OGRE_LIBRARY_DIR%\OgreTerrain.lib;%OGRE_LIBRARY_DIR%\OgrePaging.lib

@set INCLUDE=%FREEIMAGE_INCLUDE_DIR%;%TBB_INCLUDE_DIR%;%INCLUDE%
@set LIB=%FREEIMAGE_LIBRARY_DIR%;%BOOST_LIBRARY_DIR%;%TBB_LIBRARY_DIR%;%LIB%

cmake -G "NMake Makefiles" -DCMAKE_PREFIX_PATH="%SDFORMAT_PATH%" -DFREEIMAGE_RUNS=1 -DPROTOBUF_SRC_ROOT_FOLDER="%PROTOBUF_PATH%" -DBOOST_ROOT:STRING="%BOOST_PATH%" -DBOOST_LIBRARYDIR:STRING="%BOOST_LIBRARY_DIR%" -DOGRE_FOUND=1 -DOGRE_VERSION=%OGRE_VERSION% -DOGRE_PLUGINDIR="%OGRE_PLUGIN_DIR%" -DOGRE_INCLUDE_DIRS="%OGRE_INCLUDE_DIR%" -DOGRE_LIBRARIES="%OGRE_LIBS%" -DOGRE-RTShaderSystem_FOUND=1 -DOGRE-RTShaderSystem_LIBRARIES="%OGRE_RTSHADERSYSTEM_LIBS%" -DOGRE-Terrain_FOUND=1 -DOGRE-Terrain_LIBRARIES="%OGRE_TERRAIN_LIBS%" -DCURL_FOUND=1 -DCURL_INCLUDEDIR="%CURL_INCLUDE_DIR%" -DCURL_LIBDIR="%CURL_LIBRARY_DIR%" -DCURL_LIBRARIES="libcurl_a" -DCMAKE_INSTALL_PREFIX="install\%build_type%" -DCMAKE_BUILD_TYPE="%build_type%" ..
