...
void MisalignmentPluginPrivate::OnUpdate(const common::UpdateInfo &_info)
{
  ...
  if (!this->targetFrameName.empty())
  {
    ...
    if (!entity)
    {
      this->targetEntity = this->world->GetEntity(this->targetFrameName);
    ...
    ignition::math::Pose3d entityFromWorld = entity->GetWorldPose().Ign();
  ...
  if (!this->referenceFrameName.empty())
  {
    ...
    if (!refEntity)
    {
      this->referenceEntity = this->world->GetEntity(
    ...
    ignition::math::Pose3d refFromWorld = refEntity->GetWorldPose().Ign();
