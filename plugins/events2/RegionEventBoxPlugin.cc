...
namespace gazebo
{
  class RegionEventBoxPlugin : public ModelPlugin
  {
    ...
    {
      std::cout << "RegionEventBoxPlugin::Load(): model=\""
                << _parent->GetName()
                << "\"" << std::endl << std::flush;
      ...
      this->poseSub = this->node->Subscribe("~/pose/info",
          &RegionEventBoxPlugin::OnPoseMsg, this);
      ...
      if (_sdf->HasElement("event"))
      {
        ...
        {
          this->eventPub = this->node->Advertise<gazebo::msgs::SimEvent>(
              "/gazebo/sim_events");
    ...
    public: void OnUpdate(const common::UpdateInfo & _info)
    {
      for (unsigned int i = 0; i < this->world->GetModelCount(); i++)
      {
        ...
        if (name == this->modelName)
        {
          ...
          {
            if (!this->UpdateSizeAndPose(model->GetSDF(),
                                         model->GetWorldPose()))
            {
              std::cerr << "RegionEventPlugin::OnUpdate(): "
                        << "size/pose update failed for \""
        ...
        std::map<std::string, common::Time>::iterator it =
            this->insiders.find(model->GetName());
    ...
    private: bool UpdateSizeAndPose(sdf::ElementPtr _sdf,
                                    const math::Pose& _pose)
    {
      std::cout << "RegionEventPlugin::UpdateSizeAndPose(): model=\""
          ...
          if (geometryEl)
          {
            ...
            if (boxEl)
            {
              if (boxEl->HasElement("size"))
              {
                ...
                math::Vector3 vmin(this->pose.pos.x - (this->size.x / 2),
                    this->pose.pos.y - (this->size.y / 2),
                    0.0);

                math::Vector3 vmax(this->pose.pos.x + (this->size.x / 2),
                    this->pose.pos.y + (this->size.y / 2),
                    this->pose.pos.z + this->size.z);
    ...
    private: math::Vector3 ParseVector3(const std::string &_str,
                                        double _scale = 1.0)
    {
      ...
      for (unsigned int i = 0; i < pieces.size(); ++i)
      {
        if (pieces[i] != "")
        {
          try
          {
            vals.push_back(_scale * boost::lexical_cast<double>(
                pieces[i].c_str()));
          }
          catch(boost::bad_lexical_cast &)
          {
            std::cerr << "xml key [" << _str << "][" << i << "] value ["
                      << pieces[i]
    ...
    private: void SendExitingRegionEvent(physics::ModelPtr model)
    {
      std::cout << "RegionEventBoxPlugin::SendExitingRegionEvent(): model=\""
                << model->GetName() << "\""" region=\"" << this->modelName
                << "\"" << std::endl << std::flush;
