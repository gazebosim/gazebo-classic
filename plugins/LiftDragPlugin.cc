...
void LiftDragPlugin::OnUpdate()
{
  ...
  ignition::math::Vector3d link_vel = this->link->WorldLinearVel(this->cp);
  ignition::math::Vector3d vel = link_vel - this->link->WorldWindLinearVel();
  ...
  if (0)
  {
    ...
    gzdbg << "Link: [" << this->link->GetName()
          << "] pose: [" << pose
          << "] dynamic pressure: [" << q << "]\n";
    gzdbg << "link spd: [" << link_vel.Length()
          << "] vel: [" << link_vel << "]\n";
    gzdbg << "air spd: [" << vel.Length()
          << "] vel: [" << vel << "]\n";
