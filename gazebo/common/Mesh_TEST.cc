...
TEST_F(MeshTest, Mesh)
{
  ...
  EXPECT_TRUE(subMesh->GetVertex(0) == ignition::math::Vector3d(1, 2, 3));

  subMesh->SetTexCoord(0, ignition::math::Vector2d(.1, .2));
  EXPECT_TRUE(subMesh->GetTexCoord(0) == ignition::math::Vector2d(.1, .2));
...
TEST_F(MeshTest, MeshMove)
{
  ...
  EXPECT_EQ(
      ignition::math::Vector3d(3.46555, 0.180391, 2.8431), mesh->GetMin());

  mesh->Center();

  EXPECT_EQ(ignition::math::Vector3d(1.0, 1.0, 1.0), mesh->GetMax());
  EXPECT_EQ(ignition::math::Vector3d(-1.0, -1.0, -1.0), mesh->GetMin());

  mesh->Translate(ignition::math::Vector3d(1, 2, 3));
  EXPECT_EQ(ignition::math::Vector3d(2.0, 3.0, 4.0), mesh->GetMax());
  EXPECT_EQ(ignition::math::Vector3d(0.0, 1.0, 2.0), mesh->GetMin());
}
...
TEST_F(MeshTest, SubMeshCenter)
{
  ...
  EXPECT_EQ(ignition::math::Vector3d(0, 1, 2), submesh.GetMin());
  EXPECT_EQ(ignition::math::Vector3d(2, 3, 4), submesh.GetMax());

  submesh.Translate(ignition::math::Vector3d(1, 2, 3));
  EXPECT_EQ(ignition::math::Vector3d(1, 3, 5), submesh.GetMin());
  EXPECT_EQ(ignition::math::Vector3d(3, 5, 7), submesh.GetMax());

  // The original mesh should not change
  EXPECT_EQ(
      ignition::math::Vector3d(5.46554, 2.18039, 4.8431), mesh->GetMax());
  EXPECT_EQ(
      ignition::math::Vector3d(3.46555, 0.180391, 2.8431), mesh->GetMin());
}
