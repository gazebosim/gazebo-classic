...
void Link::Load(sdf::ElementPtr _sdf)
{
  ...
  {
    this->SetWindMode(this->ParentModel()->WindMode());
  }
...
void Link::Init()
{
  ...
  if (this->WindMode() && this->linkDPtr->world->WindEnabled())
...
{
  this->linkDPtr->windLinearVel =
    this->linkDPtr->world->Wind().WorldLinearVel(this);
}
...
{
  this->linkDPtr->sdf->GetElement("enable_wind")->Set(_mode);

  if (!this->WindMode() && this->linkDPtr->updateConnection)
    this->SetWindEnabled(false);
  else if (this->WindMode() && !this->linkDPtr->updateConnection)
    this->SetWindEnabled(true);
}
...
void Link::SetWindEnabled(const bool _enable)
{
  if (_enable)
  {
    this->linkDPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(
        std::bind(&Link::UpdateWind, this, std::placeholders::_1));
  }
  else
  {
    event::Events::DisconnectWorldUpdateBegin(this->linkDPtr->updateConnection);
    this->linkDPtr->updateConnection.reset();
    // Make sure wind velocity is null
    this->linkDPtr->windLinearVel.Set(0, 0, 0);
  }
}
...
{
  return this->linkDPtr->windLinearVel;
}

//////////////////////////////////////////////////
bool Link::WindMode() const
{
  return this->linkDPtr->sdf->Get<bool>("enable_wind");
}
...
void Link::SetPublishData(const bool _enable)
{
  ...
  // cleared
  if (!_enable && !this->linkDPtr->dataPub)
    return;
