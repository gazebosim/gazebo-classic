...
TEST_F(Inertial_TEST, InertialOperators)
{
  ...
  EXPECT_TRUE(i1.MOI(ignition::math::Pose3d(1, 0, 0, 0, 0, 0)) ==
      i1Offset.MOI());
  ...
  // Test GetMOI
  math::Matrix3 i11 = i1.MOI(ignition::math::Pose3d(1, 0, 0, 0, 0, 0));
  EXPECT_NEAR(i11[2][2], 1.3, TOL);
  gzdbg << "i11:\n" << i11 << "\n";

  // Get i2 from origin of link
  physics::Inertial i3;
  i3.SetMOI(i2.MOI(ignition::math::Pose3d::Zero));
  ...
  gzdbg << "R:\n" << ignition::math::Matrix3d(
      ignition::math::Quaterniond(0, 0, 0.5*M_PI)) << "\n";
  gzdbg << "I:\n" << i2.MOI() << "\n";
  i2.SetMOI(i2.MOI(ignition::math::Pose3d(0, 0, 1, 0, 0, 0.5*M_PI)));
  gzdbg << "i2:\n" << i2 << "\n";
