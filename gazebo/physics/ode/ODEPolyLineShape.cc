...
#include "gazebo/physics/ode/ODEPolyLineShape_new.hh"
...
//////////////////////////////////////////////////
ODEPolyLineShape::ODEPolyLineShape(CollisionPtr _parent) : PolyLineShape(_parent)
{
  this->odeData = NULL;
  this->vertices = NULL;
  this->indices = NULL;
...
ODEPolyLineShape::~ODEPolyLineShape()
{
  ...
  delete [] this->indices;
  dGeomTriPolyLineDataDestroy(this->odeData);
}
...
void ODEPolyLineShape::Update()
{
  ...
  dGeomPolyLineSetLastTransform(ocollision->GetCollisionId(),
...
void ODEPolyLineShape::Init()
{
  ...
  if (this->odeData == NULL)
    this->odeData = dGeomPolyLineDataCreate();
  ...
  dGeomPolyLineDataBuildSingle(this->odeData,
  ...
  if (pcollision->GetCollisionId() == NULL)
  {
    ...
    pcollision->SetCollision(dCreateTriPolyLine(pcollision->GetSpaceId(),
          this->odeData, 0, 0, 0), true);
  }
  else
  {
    dGeomPolyLineSetData(pcollision->GetCollisionId(), this->odeData);
  }
