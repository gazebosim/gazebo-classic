...
{
  auto odeChild = std::dynamic_pointer_cast<ODELink>(
      this->odeJointDPtr->childLink);
  auto odeParent = std::dynamic_pointer_cast<ODELink>(
      this->odeJointDPtr->parentLink);

  Joint::Detach();
  this->odeJointDPtr->childLink.reset();
  this->odeJointDPtr->parentLink.reset();
...
void ODEJoint::Reset()
{
  ...
  this->odeJointDPtr->forceAppliedTime = common::Time::Zero;
