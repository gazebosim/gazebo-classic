...
void World::Run(unsigned int _iterations)
{
  ...
  DIAG_TIMER_START("World::Run create thread World::RunLoop");
  this->dataPtr->thread = new boost::thread(boost::bind(&World::RunLoop, this));
  DIAG_TIMER_STOP("World::Run create thread World::RunLoop");
}
...
void World::Stop()
{
  ...
  {
    DIAG_TIMER_START("World::Stop destroy thread World::RunLoop");
    this->dataPtr->thread->join();
    delete this->dataPtr->thread;
    this->dataPtr->thread = NULL;
    DIAG_TIMER_STOP("World::Stop destroy thread World::RunLoop");
  }
}
...
void World::RunLoop()
{
  ...
  DIAG_TIMER_START("World::RunLoop create thread World::LogWorker");
  this->dataPtr->logThread =
    new boost::thread(boost::bind(&World::LogWorker, this));
  DIAG_TIMER_STOP("World::RunLoop create thread World::LogWorker");
  ...
  {
    DIAG_TIMER_START("World::RunLoop destroy thread World::LogWorker");
    ...
    this->dataPtr->logThread = NULL;
    DIAG_TIMER_STOP("World::RunLoop destroy thread World::LogWorker");
  }
}
