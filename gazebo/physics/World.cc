...
void World::Step()
{
  ...
  double updatePeriod = this->dataPtr->physicsEngine->UpdatePeriod();
      ...
      this->dataPtr->sleepOffset >= common::Time(updatePeriod))
  {
    ...
    double stepTime = this->dataPtr->physicsEngine->MaxStepSize();
...
void World::Update()
{
  ...
  if (this->dataPtr->enablePhysicsEngine && this->dataPtr->physicsEngine)
  {
    ...
    //           and we need to propagate it into Entity::worldPose
    {
      ...
      std::lock_guard<std::recursive_mutex> lock(
        this->dataPtr->physicsEngine->PhysicsUpdateMutex());
  ...
  this->dataPtr->physicsEngine->ContactMgr()->PublishContacts();
...
void World::ClearModels()
{
  ...
  for (auto &model : this->dataPtr->models)
  {
    this->dataPtr->rootElement->RemoveChild(model->Id());
  }
  this->dataPtr->models.clear();
...
{
  if (this->dataPtr->rootElement)
    return BasePtr(this->dataPtr->rootElement->BaseByName(_name));
  else
    return BasePtr();
}
...
ModelPtr World::ModelById(unsigned int _id) const
{
  return std::dynamic_pointer_cast<Model>(
      this->dataPtr->rootElement->BaseById(_id));
}
...
{
  return std::dynamic_pointer_cast<Entity>(this->BaseByName(_name));
}
...
ModelPtr World::LoadModel(sdf::ElementPtr _sdf , BasePtr _parent)
{
  ...
  if (_sdf->GetName() == "model")
  {
    ...
    event::Events::addEntity(model->ScopedName());
...
ActorPtr World::LoadActor(sdf::ElementPtr _sdf , BasePtr _parent)
{
  ...
  event::Events::addEntity(actor->ScopedName());
...
void World::BuildSceneMsg(msgs::Scene &_scene, BasePtr _entity)
{
  if (_entity)
  {
    ...
    for (unsigned int i = 0; i < _entity->ChildCount(); ++i)
    {
      this->BuildSceneMsg(_scene, _entity->Child(i));
    }
  }
}
  ...
  for (unsigned int i = 0; i < this->dataPtr->rootElement->ChildCount(); i++)
  {
    if (this->dataPtr->rootElement->Child(i)->HasType(Base::MODEL))
    {
      ModelPtr model = std::static_pointer_cast<Model>(
          this->dataPtr->rootElement->Child(i));
      model->LoadPlugins();
    }
  }
}
  ...
  for (auto const &requestMsg : this->dataPtr->requestMsgs)
  {
    ...
    if (requestMsg.request() == "entity_list")
    {
      ...
      for (unsigned int i = 0;
          i < this->dataPtr->rootElement->ChildCount(); ++i)
      {
        BasePtr entity = this->dataPtr->rootElement->Child(i);
    ...
    {
      BasePtr entity(
        this->dataPtr->rootElement->BaseByName(requestMsg.data()));
      if (entity)
      {
        if (entity->HasType(Base::MODEL))
        {
...

  {
    ...
    for (auto const &factoryMsg : this->dataPtr->factoryMsgs)
    {
      ...
      else if (factoryMsg.has_clone_model_name())
      {
        ...
        std::string newName = model->Name() + "_clone";
        int i = 0;
        while (this->ModelByName(newName))
        {
          newName = model->Name() + "_clone_" + std::to_string(i);
          i++;
        }
      ...
      {
        BasePtr base(
          this->dataPtr->rootElement->BaseByName(factoryMsg.edit_name()));
        if (base)
        {
          sdf::ElementPtr elem;
...
ModelPtr World::ModelBelowPoint(const ignition::math::Vector3d &_pt) const
{
  ...
  if (entity)
    model.reset(entity->ParentModel());
...
{
  this->SetSimTime(_state.SimTime());
  ...
  {
    ModelPtr model = this->ModelByName(modelState.second.Name());
    if (model)
  ...
  {
    LightPtr light = this->LightByName(lightState.second.Name());
    if (light)
      ...
      if (!this->dataPtr->publishModelPoses.empty())
      {
        for (auto const &model : this->dataPtr->publishModelPoses)
        {
          ...
          while (!modelList.empty())
          {
            ...
            poseMsg->set_name(m->ScopedName());
            poseMsg->set_id(m->Id());
            msgs::Set(poseMsg, m->RelativePose());

            // Publish each of the model's child links relative poses
            Link_V links = m->Links();
            for (auto const &link : links)
            {
              poseMsg = msg.add_pose();
              poseMsg->set_name(link->ScopedName());
              poseMsg->set_id(link->Id());
  ...
  // Process light poses after light factory
  {
        ...
        this->dataPtr->lightPub->HasConnections())
    {
      for (auto const &light : this->dataPtr->publishLightPoses)
      {
        ...
        msgs::Light lightMsg;
        lightMsg.set_name(light->ScopedName());
...
{
  std::lock_guard<std::recursive_mutex> plock(
      this->Physics()->PhysicsUpdateMutex());
  ...
  // Remove all the dirty poses from the delete entity.
  {
             ...
             entity != this->dataPtr->dirtyPoses.end(); ++entity)
    {
      if ((*entity)->Name() == _name ||
         ((*entity)->Parent() && (*entity)->Parent()->Name() == _name))
      {
  ...
  if (this->dataPtr->sdf->HasElement("light"))
  {
    ...
    if (childElem)
    {
      ...
      {
        if ((*light)->Name() == _name || (*light)->ScopedName() == _name)
        {
...

  {
    ...
    {
      if ((*model)->Name() == _name || (*model)->ScopedName() == _name)
      {
