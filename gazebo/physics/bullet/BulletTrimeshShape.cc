...
void BulletTrimeshShape::Init()
{
  ...
  {
    vertices[j*3+0] = vertices[j*3+0] * this->sdf->Get<math::Vector3>("scale").x;
    vertices[j*3+1] = vertices[j*3+1] * this->sdf->Get<math::Vector3>("scale").y;
    vertices[j*3+2] = vertices[j*3+2] * this->sdf->Get<math::Vector3>("scale").z;
  }
