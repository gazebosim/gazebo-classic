...
void DARTPhysics::Reset()
{
  ...
  {
    dartModelIt =
      std::dynamic_pointer_cast<DARTModel>(this->world->ModelByIndex(i));
...
void DARTPhysics::UpdatePhysics()
{
  ...
  for (unsigned int i = 0; i < modelCount; ++i)
  {
    ...
    {
      dartLinkItr
          = std::dynamic_pointer_cast<DARTLink>(links.at(j));
                                    ...
                                    CollisionPtr _collision)
{
  ...
  DARTCollisionPtr collision =
    std::dynamic_pointer_cast<DARTCollision>(_collision);
    ...
    const dart::dynamics::BodyNode *_dtBodyNode)
{
       ...
       itModel != models.end(); ++itModel)
  {
    ...
    {
      DARTLinkPtr dartLink = std::dynamic_pointer_cast<DARTLink>(*itLink);
