...
public:
	QLayout *variableLayout;

	/// \brief Variables inside this container
public:
	std::map<unsigned int, VariablePill*> variables;
...
public:
	VariablePill *selectedVariable = nullptr;
};
}
}
...
VariablePillContainer::VariablePillContainer(QWidget *_parent) :
		QWidget(_parent), dataPtr(new VariablePillContainerPrivate) {
	// label
...
unsigned int VariablePillContainer::AddVariablePill(const std::string &_name,
	...
	if (this->dataPtr->maxSize != -1
			&& static_cast<int>(this->VariablePillCount())
	...
	if (_targetId != VariablePill::EmptyVariable
		...
		gzerr << "Unable to add variable. Target variable not found"
				<< std::endl;
		return VariablePill::EmptyVariable;
	}
	...
	connect(variable, SIGNAL(VariableMoved(unsigned int)), this,
			SLOT(OnMoveVariable(unsigned int)));
	connect(variable, SIGNAL(VariableAdded(unsigned int, std::string)), this,
			SLOT(OnAddVariable(unsigned int, std::string)));
	connect(variable, SIGNAL(VariableRemoved(unsigned int)), this,
			SLOT(OnRemoveVariable(unsigned int)));
	connect(variable, SIGNAL(VariableLabelChanged(std::string)), this,
			SLOT(OnSetVariableLabel(std::string)));
...
bool VariablePillContainer::IsDragValid(QDropEvent *_evt) const {
	...
	if (_evt->mimeData()->hasFormat("application/x-item")) {
		...
		VariablePill *dragVariable = qobject_cast<VariablePill*>(
				_evt->source());
		if (!dragVariable)
			return false;
	...
	for (const auto v : this->dataPtr->variables) {
		...
		for (const auto cv : v.second->VariablePills()) {
			...
			if (!selected) {
				...
				ignition::math::Vector2i childPt(childPoint.x(),
						childPoint.y());
