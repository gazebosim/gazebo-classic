...
public:
	IncrementalPlot *plot = nullptr;

	/// \brief A map of container variable ids to their plot curve ids.
public:
	std::map<unsigned int, unsigned int> variableCurves;
};
...
public:
	std::map<unsigned int, PlotData*> plotData;

	/// \brief Pointer to an empty plot.
public:
	IncrementalPlot *emptyPlot = nullptr;

	/// \brief Container for all the variableCurves on the Y axis.
public:
	VariablePillContainer *yVariableContainer = nullptr;

	/// \brief Delete canvas Qt action
public:
	QAction *deleteCanvasAct = nullptr;

	/// \brief Global plot counter.
public:
	static unsigned int globalPlotId;
};
}
}
...
PlotCanvas::PlotCanvas(QWidget *_parent) :
		QWidget(_parent), dataPtr(new PlotCanvasPrivate()) {
	...
	connect(showHoverLineAct, SIGNAL(triggered()), this,
			SLOT(OnShowHoverLine()));
	...
	xVariableContainer->setSizePolicy(QSizePolicy::Expanding,
			QSizePolicy::Fixed);
	...
	this->dataPtr->yVariableContainer->setSizePolicy(QSizePolicy::Expanding,
			QSizePolicy::Fixed);
	...
	plotScrollArea->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Expanding);
	...
	connect(this->dataPtr->emptyPlot, SIGNAL(VariableAdded(std::string)), this,
...
void PlotCanvas::AddVariable(const unsigned int _id,
	...
	if (common::URI::Valid(_variable)) {
		...
		if (schemeStr == "data") {
			...
			std::string label = PlotManager::Instance()->HumanReadableName(
					_variable);
...
void PlotCanvas::OnMoveVariable(const unsigned int _id,
	...
	if (plotIt != this->dataPtr->plotData.end()) {
		...
		if (targetPlotIt != this->dataPtr->plotData.end()) {
			...
			auto it = this->dataPtr->plotData.find(plotId);
			GZ_ASSERT(it != this->dataPtr->plotData.end(),
					"Failed to add new plot");
...
void PlotCanvas::Restart() {
	...
	// variable to.
	std::vector<std::tuple<std::string, VariablePill*, unsigned int>> variableCurvesToClone;
	...
	for (const auto &it : this->dataPtr->plotData) {
		for (const auto &v : it.second->variableCurves) {
			...
			VariablePill *variablePill =
					this->dataPtr->yVariableContainer->GetVariablePill(
							variableId);
			...
			if (c->Active()) {
				...
				variableCurvesToClone.push_back(
						std::make_tuple(variablePill->Text(), variablePill,
								it.first));
			}
			// increment curve age.
			c->SetAge(c->Age() + 1);
...
void PlotCanvas::UpdateAxisLabel() {
	...
	for (int i = 0; i < this->dataPtr->plotSplitter->count(); ++i) {
		IncrementalPlot *p = qobject_cast<IncrementalPlot*>(
				this->dataPtr->plotSplitter->widget(i));
...
void PlotCanvas::ExportPDF(const std::string &_filePrefix) const {
	...
	for (const auto it : this->dataPtr->plotData) {
		...
		std::string filename = common::unique_file_path(_filePrefix + suffix,
				"pdf");
...
void PlotCanvas::ExportCSV(const std::string &_filePrefix) const {
	...
	for (const auto it : this->dataPtr->plotData) {
		for (const auto &curve : it.second->plot->Curves()) {
			...
			std::string filename = common::unique_file_path(
					_filePrefix + "-" + label, "csv");
