  ...
  if (imuElem->HasElement("noise"))
  {
    ...
    if (type == "gaussian")
    {
      ...
      if (noiseElem->HasElement("rate"))
      {
        ...
        this->rateBias = ignition::math::Rand::DblNormal(
...
//////////////////////////////////////////////////
ignition::math::Vector3d ImuSensor::GetAngularVelocity() const
{
  boost::mutex::scoped_lock lock(this->mutex);
...
//////////////////////////////////////////////////
ignition::math::Vector3d ImuSensor::GetLinearAcceleration() const
{
  boost::mutex::scoped_lock lock(this->mutex);
...
//////////////////////////////////////////////////
ignition::math::Quaterniond ImuSensor::GetOrientation() const
{
  boost::mutex::scoped_lock lock(this->mutex);
...
bool ImuSensor::UpdateImpl(bool /*_force*/)
{
  ...
  if (dt > 0.0)
  {
    ...
    msgs::Set(this->imuMsg.mutable_angular_velocity(),
              imuPose.Rot().Inverse().RotateVector(
              imuWorldAngularVel));
