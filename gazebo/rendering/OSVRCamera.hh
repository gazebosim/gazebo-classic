...
#ifndef _GAZEBO_OSVR_CAMERA_HH_
#define _GAZEBO_OSVR_CAMERA_HH_
...
namespace gazebo
{
  namespace rendering
  {
    class OSVRCameraPrivate;

    /// \addtogroup gazebo_rendering
    /// \{

    /// \class OSVRCamera OSVRCamera.hh rendering/rendering.hh
    /// \brief A camera used for user visualization of a scene
    class GZ_RENDERING_VISIBLE OSVRCamera : public Camera
    {
      /// \brief Constructor
      /// \param[in] _name Name of the camera.
      /// \param[in] _scene Scene to put the camera in.
      public: OSVRCamera(const std::string &_name, ScenePtr _scene);

      /// \brief Destructor
      public: virtual ~OSVRCamera();
