...
namespace SkyX
{
  ...
  void MoonManager::updateMoonPhase(const Ogre::Real& phase)
  {
    ...
    mMoonMaterial->getTechnique(0)->getPass(0)->
      getFragmentProgramParameters()->setNamedConstant("uMoonPhase",
          Ogre::Vector3(radius, center + 0.5f, mMoonHaloStrength));
  ...
  void MoonManager::updateGeometry(Ogre::Camera* c)
  {
    ...
    mMoonBillboard->setCommonDirection(
        (mSkyX->getController()->getMoonDirection()).
        normalisedCopy().perpendicular());
