...
namespace VClouds
{
      ...
      const Ogre::Real& timeSinceLastCameraFrame)
  {
    ...
    mSceneNode->setPosition(
        mVClouds->getCamera()->getDerivedPosition().x,
        mVClouds->getCamera()->getDerivedPosition().z,
        mHeight.x);
  ...
  void GeometryManager::_createGeometry()
  {
    ...
    for (int k = 0; k < mNumberOfBlocks; k++)
    {
      mGeometryBlocks.push_back(
          new GeometryBlock(mVClouds, mHeight.y, mAlpha, mBeta, mRadius,
            mPhi, mNa, mNb, mNc, mA, mB, mC, k));

      mGeometryBlocks.at(k)->create();
      // Each geometry block must be in a different scene node, See:
      // GeometryBlock::isInFrustum(Ogre::Camera *c)
      ...
      const Ogre::Real& timeSinceLastFrame)
  {
    ...
    Ogre::Vector2 CameraDirection = Ogre::Vector2(
        c->getDerivedDirection().x, c->getDerivedDirection().y);

    float offset = - CameraDirection.dotProduct(mVClouds->getWindDirectionV2())
      * mVClouds->getWindSpeed() * timeSinceLastFrame;
    ...
    for (int k = 0; k < mNumberOfBlocks; k++)
    {
      mGeometryBlocks.at(k)->setWorldOffset(mWorldOffset +
          currentCameraData.cameraOffset);
