...
namespace SkyX
{
  namespace VClouds
  {
    ...
    void DataManager::update(const Ogre::Real &timeSinceLastFrame)
    {
      if (mVolTexToUpdate)
      {
        ...
        else
        {
          if (mXStart != mXEnd)
          {
            _performCalculations(mNx, mNy, mNz, mXEnd/mNx, mXStart%mNx,
                mXEnd%mNx);
          }
        }
    ...
    void DataManager::_delete3DCellArray(Cell ***c, const int& nx,
                                         const int& ny)
    {
      int u, v;
      ...
      while (static_cast<unsigned int>(numberofclouds) > mEllipsoids.size())
      {
        newclouddimensions = maxcloudsize *
          Ogre::Vector3(Ogre::Math::RangeRandom(0.5, 2),
                        Ogre::Math::RangeRandom(0.5, 2),
                        Ogre::Math::RangeRandom(0.75, 1));
    ...
    void DataManager::addEllipsoid(Ellipsoid *e,
        ...
        const float& att) const
    {
      ...
      while (!outOfBounds)
      {
        if (static_cast<int>(pos.z) >= nz || static_cast<int>(pos.z) < 0 ||
            factor <= 0 || current_iteration >= max_iterations)
        {
          outOfBounds = true;
        }
        else
        {
          ...
          factor -= c[uu][vv][static_cast<int>(pos.z)].dens * att *
            (1 - static_cast<float>(current_iteration) / max_iterations);
          pos += step * (-d);
        ...
        case 3:
          {
            ...
            for (u = xStart; u < xEnd; u++)
            {
              for (v = 0; v < ny; v++)
              {
                for (w = 0; w < nz; w++)
                {
                  mCellsCurrent[u][v][w].light =
                    _getLightAbsorcionAt(mCellsCurrent, nx, ny, nz, u, v, w,
                        SunDir, 0.15f/*TODO!!!!*/);
                }
              }
            }
    ...
    void DataManager::_updateVolTextureData(Cell ***c,
        const VolTextureId& TexId, const int& /*nx*/, const int& /*ny*/,
        const int& /*nz*/)
    {
