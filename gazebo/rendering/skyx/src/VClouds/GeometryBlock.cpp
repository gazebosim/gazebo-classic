...
namespace SkyX { namespace VClouds
  {
    ...
    void GeometryBlock::create()
    {
      ...
      mEntity = mVClouds->getSceneManager()->createEntity(
          "_SkyX_VClouds_BlockEnt" +
          Ogre::StringConverter::toString(mPosition),
          "_SkyX_VClouds_Block" + Ogre::StringConverter::toString(mPosition));
    ...
    const Ogre::AxisAlignedBox GeometryBlock::_buildAABox(const float& fd) const
    {
      ...
      Ogre::Vector2 Min    = Ogre::Vector2(
          std::min<float>(std::min<float>(V1.x, V2.x), Center.x),
    ...
    void GeometryBlock::_createGeometry()
    {
      ...
      */
      ...
      mIndexBuffer->writeData(0, mIndexBuffer->getSizeInBytes(), indexbuffer,
            true);
    ...
    void GeometryBlock::_updateGeometry()
    {
      ...
      */
      ...
      // Upload changes
      mVertexBuffer-> writeData(0, mVertexBuffer->getSizeInBytes(), mVertices,
          true);
    }
    ...
    void GeometryBlock::_updateZoneASlice(const int& n)
    {
      ...
      Ogre::Vector2 x1 = Radius*mV2Cos;
      Ogre::Vector2 x2 = Radius*mBetaSin*mV2Cos;
      Ogre::Vector2 y1 = Radius*mV2Sin;
      Ogre::Vector2 y2 = Radius*mBetaSin*mV2Sin;

      float z0 = Radius*Ogre::Math::Sin(mAlpha);

      // Vertex 0
      _setVertexData(VertexOffset, Ogre::Vector3(x1.x, y1.x, 0), opacity);
      // Vertex 1
      _setVertexData(VertexOffset+1, Ogre::Vector3(x1.y, y1.y, 0), opacity);
      // Vertex 2
      _setVertexData(VertexOffset+2, Ogre::Vector3(x2.x, y2.x, z0), opacity);
      // Vertex 3
      _setVertexData(VertexOffset+3, Ogre::Vector3(x2.y, y2.y, z0), opacity);

      Ogre::Vector2 x3 = Radius * mAlphaSin * mV2Cos;
      Ogre::Vector2 y3 = Radius * mAlphaSin * mV2Sin;

      float z1 = Radius*Ogre::Math::Sin(mBeta);

      // Vertex 4
      _setVertexData(VertexOffset+4, Ogre::Vector3(x3.x, y3.x, z1), opacity);
      // Vertex 5
      _setVertexData(VertexOffset+5, Ogre::Vector3(x3.y, y3.y, z1), opacity);
    ...
    void GeometryBlock::_setVertexData(const int& index,
        const Ogre::Vector3& p, const float& o)
    {
      float fallingDistance = mVClouds->getDistanceFallingParams().x *
        (mEntity->getParentSceneNode()->_getDerivedPosition().z -
      ...
      mVertices[index].y = p.y;
      mVertices[index].z = p.z - fallingDistance;

      // 3D coords (Z-UP)
      float scale = mVClouds->getCloudFieldScale() / mRadius;
      mVertices[index].xc = (p.x+mWorldOffset.x) * scale;
      mVertices[index].yc = (p.y+mWorldOffset.y) * scale;
      mVertices[index].zc = Ogre::Math::Clamp<Ogre::Real>(p.z / mHeight, 0, 1);

      // Noise coords
      float noise_scale = mVClouds->getNoiseScale() / mRadius;
      float xz_length_radius = Ogre::Vector2(p.x, p.y).length() / mRadius;

      Ogre::Vector3 origin = Ogre::Vector3(0, 0,
          -(mEntity->getParentSceneNode()->_getDerivedPosition().z -
        ...
        const Ogre::Vector3& p, const float& o)
    {
      ...
      Ogre::Vector3 origin = Ogre::Vector3(0, 0,
          -(mEntity->getParentSceneNode()->_getDerivedPosition().z -
            mCamera->getDerivedPosition().z) - mRadius *
          (0.5f + 0.5f * Ogre::Vector2(p.x, p.z).length()/mRadius));

      //Ogre::Vector3 dir = (p-origin).normalisedCopy();
      Ogre::Vector3 dir = (Ogre::Vector3(p.x, p.z, p.y) -
          origin).normalisedCopy();
      ...
      mVertices[index].v = (uv.y+mWorldOffset.y) * noise_scale;
