...
namespace SkyX { namespace VClouds
{
  Lightning::Lightning(Ogre::SceneManager* sm, Ogre::SceneNode* sn,
      const Ogre::Vector3& _or, const Ogre::Vector3& dir, const Ogre::Real& l,
      const Ogre::uint32& d, const Ogre::uint32& rec, const Ogre::Real& tm,
      const Ogre::Real& wm, const Ogre::Vector2& b)
    : mOrigin(_or)
    , mDirection(dir)
    , mLength(l)
    , mRealLength(0)
  ...
  void Lightning::create()
  {
    ...
    Ogre::Real width = mWidthMultiplier * 3 *
      (static_cast<Ogre::Real>(mRecursivity) / 4 + 1) *
      Ogre::Math::RangeRandom(0.5f, 2.5f - mRecursivity / 3);
    ...
    for (Ogre::uint32 k = 0; k < mSegments.size(); ++k)
    {
      ...
      bb = mBillboardSet->createBillboard((mSegments.at(k).a +
            mSegments.at(k).b)/2);
    ...
    if (mRecursivity > 0)
    {
      ...
      {
        angle = (mSegments.at(k).b -
            mSegments.at(k).a).normalisedCopy().dotProduct(
        ...
        if (angle < Ogre::Math::RangeRandom(mAngleRange.x, mAngleRange.y))
        {
          ...
          delta = 1.0f / mSegments.size();
          bounds = Ogre::Vector2(mBounds.x + (mBounds.y - mBounds.x) *
              (k + 1) * delta, 1);

          lengthMult = Ogre::Math::RangeRandom(0.1f, 0.7f);

          Lightning* lightning = new Lightning(mSceneManager, mSceneNode,
              mSegments.at(k).b, dir, lengthMult * mLength,
  ...
  void Lightning::update(Ogre::Real timeSinceLastFrame)
  {
    ...
    if (mTime < 1)
    {
      ...
      if (mTime > 2)
        mTime = 1.5f;  // Prevent big changes
    ...
    else if (mTime > 1 && mTime < 2)
    {
      ...
      {
        alpha += Ogre::Math::Abs(
            Ogre::Math::Sin((mTime - 1.2f) * 1.5f * mTimeMultipliers.x));
      }
    }
