...
void OrbitViewController::Init()
{
  ...
  if (dist < 0 || dist > 20 || ignition::math::isnan(dist))
  {
    ...
    ignition::math::Vector3d dir = this->camera->GetDirection();
    dir.Normalize();
...
void OrbitViewController::Zoom(float _amount)
{
  ...
  ignition::math::Vector3d delta = this->camera->GetWorldPosition() - this->focalPoint;
  delta.Normalize();
  delta *= this->distance;
...
void OrbitViewController::UpdateRefVisual()
{
  ...
  this->refVisual->SetScale(ignition::math::Vector3d(scale, scale, scale * 0.5));
}
