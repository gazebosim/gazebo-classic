...
{
#ifdef SSE
  __m128d d = _mm_load_pd(a+0) * _mm_load_pd(b+0) +
              _mm_load_pd(a+2) * _mm_load_pd(b+2) +
              _mm_load_pd(a+4) * _mm_load_pd(b+4);
  double r[2];
  _mm_store_pd(r, d);
  return r[0] + r[1];
#else
                ...
                boost::recursive_mutex* /*mutex*/)
{
  ...
  */
  ...
  for (int iteration=0; iteration < num_iterations; iteration++) {
    ...
    //   printf("=====> %d %d %d %f %d\n", thread_id, iteration, i,
    //     order[i].error, order[i].index);
    ...
    if ((iteration & 7) == 0) {
      ...
      for (int i=startRow+1; i<startRow+nRows; i++) {
        ...
        //        "order[%d].index=%d\n", thread_id, i, order[i].index,
        //        swapi, order[swapi].index);
...
static void SOR_LCP (dxWorldProcessContext *context,
  const int m, const int nb, dRealMutablePtr J, dRealMutablePtr J_orig,
  dRealMutablePtr vnew, int *jb, dxBody * const *body,
  dRealPtr invI, dRealPtr I, dRealMutablePtr lambda,
  dRealMutablePtr lambda_erp, dRealMutablePtr caccel,
  dRealMutablePtr caccel_erp, dRealMutablePtr cforce,
  dRealMutablePtr rhs, dRealMutablePtr rhs_erp,
  ...
  const dReal stepsize)
{
  ...
  //printf("      quickstep done scheduling start time %f stopped time "
  ...
  dxJoint * const *_joint, int _nj, dReal stepsize)
{
...

  {
    ...
    for (dxBody *const *bodycurr = body; bodycurr != bodyend;
      ...
      // also store I for later (used by preconditioner)
  ...
  if (m > 0) {
    dReal *cfm, *lo, *hi, *rhs, *rhs_erp, *Jcopy;
    dReal *c_v_max;
    int *findex;

    {
      ...
      // later, over-written by contact.cpp when getInfo2 is called
    ...
    BEGIN_STATE_SAVE(context, lcpstate) {
      ...
      SOR_LCP (context, m, nb, J, J_orig, vnew, jb, body,
    ...
    for (dxBody *const *bodycurr = body; bodycurr != bodyend;
      ...
      for (int j=0; j<3; j++) {
        ...
        */
        ...
        printf("nb[%d] m[%d] b[%d] i[%d] v[%f] dv[%f] vf[%f] a[%f] "
               "da[%f] af[%f] debug[%f - %f][%f - %f]\n"
  ...
  dxBody * const * /*body*/, int nb, dxJoint * const *_joint, int _nj)
{
  ...
  res += dEFFICIENT_SIZE(sizeof(dReal) * 3 * 4 * nb); // for I needed by precon
