    ...
    const T &_epsilon = 1e-6)
{
  ...
  void MeshDecimator::Initialize(size_t nVertices, size_t nTriangles,   Vec3<Float> * points,  Vec3<int32_t> * triangles)
  {
    ...
    long nEdges = 0;
    long idEdge;
    int32_t nTris = static_cast<int32_t>(m_nTriangles);
    for(int32_t t = 0; t < nTris; ++t)
    {
  ...
  void MeshDecimator::EdgeCollapse(long v1, long v2)
  {
    ...
    {
      incidentVertices.PushBack((m_edges[idEdge].m_v1!= v1) ?
          m_edges[idEdge].m_v1 : m_edges[idEdge].m_v2);
  ...
  void MeshDecimator::GetMeshData(Vec3<Float> * points, Vec3<int32_t> * triangles) const
  {
  ...
  double MeshDecimator::ComputeEdgeCost(long v1, long v2, Vec3<Float> & newPos) const
  {
    ...
    SArray<int32_t, SARRAY_DEFAULT_MIN_SIZE> triangles = m_vertices[v1].m_triangles;
    long idTriangle;
    for(size_t itT = 0; itT < m_vertices[v2].m_triangles.Size(); ++itT)
    {
      idTriangle = m_vertices[v2].m_triangles[itT];
  ...
  bool MeshDecimator::ManifoldConstraint(long v1, long v2) const
  {
    ...
    if (vertices.size() <= 4 ||
        ( m_vertices[v1].m_onBoundary && m_vertices[v2].m_onBoundary && !m_edges[idEdgeV1V2].m_onBoundary))
    {
      return false;
    }
    return true;
  ...
  bool MeshDecimator::EdgeCollapse(double & qem)
  {
    ...
    while ( (!m_edges[currentEdge.m_name].m_tag) ||
        !equal(m_edges[currentEdge.m_name].m_qem, currentEdge.m_qem));
  ...
  bool MeshDecimator::Decimate(size_t targetNVertices, size_t targetNTriangles, double targetError)
  {
    ...
    if (m_callBack)
    {
      ...
      msg_ << "+ Mesh" << std::endl;
      msg_ << "\t # vertices                     \t" << m_nPoints << std::endl;
      msg_ << "\t # triangles                    \t" << m_nTriangles << std::endl;
      msg_ << "+ Parameters" << std::endl;
      msg_ << "\t target # of vertices           \t" << targetNVertices << std::endl;
      msg_ << "\t target # of triangles          \t" << targetNTriangles << std::endl;
      msg_ << "\t QEM                        \t" << targetError << std::endl;
      (*m_callBack)(msg_.str().c_str(), 0.0, 0.0, m_nPoints);
    }
    ...
    {
      std::ostringstream msg_;
      msg_ << "+ Simplification output" << std::endl;
      msg_ << "\t # vertices                     \t" << m_nVertices << std::endl;
      msg_ << "\t # triangles                    \t" << m_nTriangles << std::endl;
      msg_ << "\t QEM                    \t" << qem << std::endl;
      (*m_callBack)(msg_.str().c_str(), 100.0, qem, m_nVertices);
    }
    return true;
  }
}
