...
if [ $xmlout -eq 1 ]; then
  ...
  (cppcheck --xml --enable=style,performance,portability,information --rule-file=./tools/issue_581.rule -j 4 -q --suppressions-list=/tmp/gazebo_cpp_check.suppress `find ./plugins ./gazebo ./tools ./examples ./test/regression ./interfaces -name "*.cc"`) 2> $xmldir/cppcheck.xml 
  ...
  # Run most of the checks in parallel
  cppcheck --enable=style,performance,portability,information --rule-file=./tools/issue_581.rule -j 4 -q --suppressions-list=/tmp/gazebo_cpp_check.suppress `find ./plugins ./gazebo ./tools ./examples ./test/regression ./interfaces -name "*.cc"` 
