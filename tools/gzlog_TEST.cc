...
/////////////////////////////////////////////////
/// Check to make sure that 'ignlog info' returns correct information
TEST(ignlog, Info)
{
  std::string info = custom_exec(std::string("ignlog info ") +
...
/////////////////////////////////////////////////
/// Check to make sure that 'ignlog echo' returns correct information
TEST(ignlog, Echo)
{
  std::string echo = custom_exec(std::string("ignlog echo ") +
...
/////////////////////////////////////////////////
/// Check to make sure that 'ignlog echo --filter' returns correct information
TEST(ignlog, EchoFilter)
{
  std::string echo, shasum;

  // Test model filter
  echo = custom_exec(
      std::string("ignlog echo --filter pr2 ") +
      PROJECT_SOURCE_PATH + "/test/data/pr2_state.log");
  shasum = ignition::common::get_sha1<std::string>(echo);
  // EXPECT_EQ(pr2StateLog, echo);
  EXPECT_EQ(shasum, "0bf1f293b164bbe820267f970c4b419acdca4b01");

  echo = custom_exec(
      std::string("ignlog echo --filter pr2.pose ") +
      PROJECT_SOURCE_PATH + "/test/data/pr2_state.log");
  shasum = ignition::common::get_sha1<std::string>(echo);
  ...
  echo = custom_exec(
      std::string("ignlog echo --filter pr2.pose.x ") +
      PROJECT_SOURCE_PATH + "/test/data/pr2_state.log");
  shasum = ignition::common::get_sha1<std::string>(echo);
  ...
  echo = custom_exec(
      std::string("ignlog echo --filter pr2.pose.[x,y] ") +
      PROJECT_SOURCE_PATH + "/test/data/pr2_state.log");
  shasum = ignition::common::get_sha1<std::string>(echo);
  ...
  // Test link filter
  echo = custom_exec(
      std::string("ignlog echo --filter pr2/r_upper*.pose ") +
      PROJECT_SOURCE_PATH + "/test/data/pr2_state.log");
  shasum = ignition::common::get_sha1<std::string>(echo);
  ...
  // Test joint filter
  echo = custom_exec(
      std::string("ignlog echo --filter pr2//r_upper_arm_roll_joint ") +
      PROJECT_SOURCE_PATH + "/test/data/pr2_state.log");
  shasum = ignition::common::get_sha1<std::string>(echo);
...
TEST(ignlog, HzFilter)
{
  std::string echo, validEcho;

  // Test Hz filter
  echo = custom_exec(
      std::string("ignlog echo -r -z 1.0 --filter pr2.pose.z ") +
      PROJECT_SOURCE_PATH + "/test/data/pr2_state.log");
  boost::trim_right(echo);
  validEcho = "-0.000008";
  EXPECT_EQ(validEcho, echo);
  ...
  // Test zero Hz filter
  echo = custom_exec(
      std::string("ignlog echo -r -z 0 --filter pr2.pose.z ") +
      PROJECT_SOURCE_PATH + "/test/data/pr2_state.log");
  boost::trim_right(echo);
  validEcho = "-0.000008 \n-0.000015";
  EXPECT_EQ(validEcho, echo);
...
/////////////////////////////////////////////////
/// Check to make sure that 'ignlog step' returns correct information
/// Just check number of characters returned for now
TEST(ignlog, Step)
{
  std::string stepCmd, shasum;
  stepCmd = std::string("ignlog step ") + PROJECT_SOURCE_PATH +
    std::string("/test/data/pr2_state.log");

  // Call ignlog step and press q immediately
  std::string stepq0 = custom_exec(std::string("echo 'q' | ") + stepCmd);
  shasum = ignition::common::get_sha1<std::string>(stepq0);
  EXPECT_EQ(shasum, "6d3af4f4d1214fe3a4860ab42777eb4d0f89c6b2");

  // Call ignlog step and press space once, then q
  std::string stepq1 = custom_exec(std::string("echo ' q' | ") + stepCmd);
  shasum = ignition::common::get_sha1<std::string>(stepq1);
  EXPECT_EQ(shasum, "43eacb140e00ef0525d54667bc558d63dac3d21f");

  // Call ignlog step and press space twice, then q
  std::string stepq2 = custom_exec(std::string("echo '  q' | ") + stepCmd);
  shasum = ignition::common::get_sha1<std::string>(stepq2);
  EXPECT_EQ(shasum, "37e133d15d3f74cbc686bfceb26b8db46e2f6bf5");
}

/////////////////////////////////////////////////
TEST(ignlog, HangCheck)
{
  ignition::common::Time start = ignition::common::Time::GetWallTime();
  custom_exec("ignlog stop");
  ignition::common::Time end = ignition::common::Time::GetWallTime();

  EXPECT_LT(end - start, ignition::common::Time(60, 0));
}
