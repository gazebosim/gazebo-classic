...
TEST_F(Issue1124Test, SetModelPose)
{
  ...
  EXPECT_EQ(coll->GetWorldPose(), ignition::math::Pose3d(0, 0, 0.5, 0, 0, 0));
  ...
  EXPECT_EQ(model->GetWorldPose(), ignition::math::Pose3d(2, 2, 0.5, 0, 0, 0));
  world->Step(1);

  EXPECT_EQ(link->GetWorldPose(), ignition::math::Pose3d(2, 2, 0.5, 0, 0, 0));
  world->Step(1);

  // The new pose should be centered a the new model location
  EXPECT_EQ(coll->GetWorldPose(), ignition::math::Pose3d(2, 2, 0.5, 0, 0, 0));
  world->Step(1);
}
...
TEST_F(Issue1124Test, SetLinkPose)
{
  ...
  // The new pose should be centered a the new model location
  EXPECT_EQ(coll->GetWorldPose(), ignition::math::Pose3d(-4, 5, 0.5, 0, 0, 0));
}
