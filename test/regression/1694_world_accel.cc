...
TEST_F(Issue1694Test, WorldAccel)
{
  ...
  physics::LinkPtr link = model->LinkByName("box_link");
  ASSERT_TRUE(link != NULL);

  const ignition::math::Vector3d g = world->Physics()->Gravity();
  ...
  CheckAccel(link->RelativeLinearAccel(), ignition::math::Vector3d::Zero);
  CheckAccel(link->WorldLinearAccel(), ignition::math::Vector3d::Zero);
}
