...
void WorldOpenTest::NewWorldMsg(const std::string &_physicsEngine)
{
  ...
  EXPECT_TRUE(world->ModelByName("ground_plane") != nullptr);
  EXPECT_TRUE(world->LightByName("sun") != nullptr);
  EXPECT_TRUE(world->ModelByName("box") != nullptr);
  EXPECT_TRUE(world->ModelByName("sphere") != nullptr);
  EXPECT_TRUE(world->ModelByName("cylinder") != nullptr);
  ...
  ASSERT_TRUE(world != nullptr);
  EXPECT_TRUE(world->SimTime() < time);
...
//    EXPECT_TRUE(world->PluginsLoaded());
    ...
    for (unsigned int j = 1; j < 10; ++j)
    {
      ...
      EXPECT_TRUE(world->ModelByName(worlds[i][j][0][0]) != nullptr);
      count++;
    }
    EXPECT_EQ(count, world->ModelCount());
    ...
    for (unsigned int j = 1; j < 10; ++j)
    {
      ...
      EXPECT_TRUE(world->LightByName(worlds[i][0][j][0]) != nullptr);
      count++;
    }
...
void WorldOpenTest::OpenWorldSensors(const std::string &_physicsEngine)
{
  ...
  // Check the sensors are there
  {
    ...
    auto camera = world->ModelByName("camera");
    EXPECT_TRUE(camera != nullptr);
    auto camera2 = world->ModelByName("camera 2");
  ...
  // Check sensor
  {
    ...
    auto model = world->ModelByName("magnetometerModel");
