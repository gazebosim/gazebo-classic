...
void PR2Test::ScrewJoint(std::string _physicsEngine)
{
  ...
  physics::LinkPtr link = model->LinkByName("torso_lift_link");
  ASSERT_TRUE(link != NULL);

  physics::JointPtr motor = model->JointByName("torso_lift_motor_screw_joint");
  ASSERT_TRUE(motor != NULL);
  motor->SetEffortLimit(0, 1000);

  physics::JointPtr screw =
    model->JointByName("torso_lift_screw_torso_lift_joint");
  ...
  gzdbg << "motor " << motor->Velocity(0) << std::endl;
  gzdbg << "screw " << screw->Velocity(0) << std::endl;
