...
void WorldEnvPopulationTest::LoadEnvironment(const std::string &/*_physicsEng*/)
{
  ...
  for (size_t i = 0; i < models.size(); ++i)
  {
    ...
    if (model->Name().find("can1_clone") != std::string::npos)
    {
    ...
    else if (model->Name().find("can3_clone") != std::string::npos)
    {
      ignition::math::Vector3d centre(-3, 3, 0);
      double radius = 1.0;
      EXPECT_LE(model->WorldPose().Pos().Distance(centre), radius + tolerance);
    }
    else if (model->Name().find("can4_clone") != std::string::npos)
    {
    ...
    else if (model->Name().find("can5_clone") != std::string::npos)
    {
      ignition::math::Vector3d centre(-3, -3, 0);
      double radius = 1.0;
      EXPECT_LE(model->WorldPose().Pos().Distance(centre), radius + tolerance);
    }
    else if (model->Name().find("can6_clone") != std::string::npos)
    {
    ...
    else if (model->Name().find("can7_clone") != std::string::npos)
    {
    ...
    else if (model->Name().find("can8_clone") != std::string::npos)
    {
