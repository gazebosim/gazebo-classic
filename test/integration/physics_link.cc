    ...
    ignition::math::Vector3d _offset)
{
  ...
  double dt = _world->Physics()->MaxStepSize();
  _world->Step(1);
...
void PhysicsLinkTest::AddForce(const std::string &_physicsEngine)
{
  ...
  EXPECT_EQ(ignition::math::Pose3d(), link->WorldPose());
  EXPECT_EQ(ignition::math::Pose3d(), link->WorldInertialPose());
  ...
  model->SetLinkWorldPose(
      ignition::math::Pose3d(
        ...
        ignition::math::Quaterniond(0, M_PI/2.0, 1)), link);
  ...
  model->SetLinkWorldPose(ignition::math::Pose3d(), link);
  EXPECT_EQ(ignition::math::Pose3d(), link->WorldPose());
  EXPECT_EQ(ignition::math::Pose3d(), link->WorldInertialPose());
  ...
  ignition::math::Pose3d inertialPose =
    ignition::math::Pose3d(ignition::math::Vector3d(1, 5, 8),
      ignition::math::Quaterniond(M_PI/3.0, M_PI*1.5, M_PI/4));
  link->Inertia().SetCoG(inertialPose);
  EXPECT_EQ(ignition::math::Pose3d(), link->WorldPose());
  ...
  gzdbg << "World != link != inertial frames, with offset" << std::endl;
  model->SetLinkWorldPose(
      ignition::math::Pose3d(ignition::math::Vector3d(5, 10, -4),
        ignition::math::Quaterniond(0, M_PI/2.0, M_PI/6)), link);
  inertialPose = ignition::math::Pose3d(ignition::math::Vector3d(0, -5, 10),
      ignition::math::Quaterniond(0, 2.0*M_PI, M_PI/3));
  link->Inertia().SetCoG(inertialPose);
  this->AddLinkForceTwoWays(world, link, ignition::math::Vector3d(1, 2, 1),
  ...
  gzdbg << "World != link != inertial frames, with offset and initial vel"
      << std::endl;
  model->SetLinkWorldPose(
      ignition::math::Pose3d(ignition::math::Vector3d(-1.5, 0.8, 3),
        ignition::math::Quaterniond(-M_PI/4.5, M_PI/3.0, M_PI*1.2)), link);
  inertialPose = ignition::math::Pose3d(ignition::math::Vector3d(1, 0, -5.6),
      ignition::math::Quaterniond(M_PI/9, 0, M_PI*3));
  link->Inertia().SetCoG(inertialPose);
  link->SetLinearVel(ignition::math::Vector3d(2, -0.1, 5));
  link->SetAngularVel(ignition::math::Vector3d(-M_PI/10, 0, 0.0001));
...
void PhysicsLinkTest::GetWorldEnergy(const std::string &_physicsEngine)
{
  ...
  // Get initial energy
  double energy0 = link->WorldEnergy();
  EXPECT_NEAR(link->WorldEnergyKinetic(), 0, g_tolerance);
  ...
  {
    world->Step(stepSize);
    double energy = link->WorldEnergy();
...
void PhysicsLinkTest::GetWorldInertia(const std::string &_physicsEngine)
{
  ...
  for (unsigned int i = 0; i < testCases; ++i)
  {
    ...
    EXPECT_EQ(link->WorldInertialPose(),
...
void PhysicsLinkTest::OnWrenchMsg(const std::string &_physicsEngine)
{
  ...
  std::string topicName = "~/" + link->ScopedName() + "/wrench";
  ...
  for (unsigned int i = 0; i < forces.size(); ++i)
  {
    ...
    ignition::math::Vector3d worldOffset = forceOffsets[i] -
      link->Inertia().CoG();
...
void PhysicsLinkTest::SetVelocity(const std::string &_physicsEngine)
{
  ...
  ignition::math::Vector3d vel3 =
    link->WorldLinearVel(offset, ignition::math::Quaterniond());
