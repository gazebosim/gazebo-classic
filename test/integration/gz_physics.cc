...
TEST_F(GzPhysics, StepSize)
{
  ...
  EXPECT_NEAR(world->Physics()->MaxStepSize(), 0.002, 1e-5);

  // Change step size
  custom_exec("gz physics -s 0.001");
  EXPECT_NEAR(world->Physics()->MaxStepSize(), 0.001, 1e-5);
}
...
TEST_F(GzPhysics, Iters)
{
  ...
  // Change iterations
  {
    ...
    boost::any iters = world->Physics()->Param("iters");
...
TEST_F(GzPhysics, UpdateRate)
{
  ...
  EXPECT_NEAR(world->Physics()->RealTimeUpdateRate(), 2.0, 1e-3);

  // Change update rate
  custom_exec("gz physics -u 0.5");
  EXPECT_NEAR(world->Physics()->RealTimeUpdateRate(), 0.5, 1e-3);
}
