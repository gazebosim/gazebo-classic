  ...
  public: physics::ModelPtr SpawnBox(const SpawnGetFTBoxOptions &_opt)
          {
            ...
            *(link->mutable_inertial()) = inertial;
            ...
            physics::JointPtr pJoint = model->JointByName("joint");
    ...
    const SpawnGetFTBoxOptions& opt)
{
  ...
  ignition::math::Vector3d g = _physics->Gravity();
  double mass = opt.mass;

  physics::ModelPtr model = SpawnBox(opt);
  physics::LinkPtr link  = model->LinkByName("link");
  ...
  {
    ASSERT_EQ(model->JointCount(), 1u);
  }
  else
  {
    ASSERT_EQ(model->JointCount(), 2u);
  }

  physics::JointWrench W = joint->ForceTorque(0u);
  ...
  if (!opt.parentIsWorld)
  {
    parentPose = link->ParentJointsLinks()[0]->WorldPose();
  }
...
void JointGetForceTorqueTest::GetForceTorqueDemo(const std::string &_physEng)
{
  ...
  EXPECT_EQ(physics->Type(), _physEng);
  ...
  // bullet and simbody ForceTorque() is
