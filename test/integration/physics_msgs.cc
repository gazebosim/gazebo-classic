...
void PhysicsMsgsTest::SetGravity(const std::string &_physicsEngine)
{
  ...
  std::vector<ignition::math::Vector3d> gravity;
  gravity.push_back(ignition::math::Vector3d(0, 0, 9.81));
  gravity.push_back(ignition::math::Vector3d(0, 0, -20));
  gravity.push_back(ignition::math::Vector3d(0, 0, 20));
  gravity.push_back(ignition::math::Vector3d(0, 0, 0));
  gravity.push_back(ignition::math::Vector3d(0, 0, -9.81));
  gravity.push_back(ignition::math::Vector3d(1, 1, 9.81));
  gravity.push_back(ignition::math::Vector3d(2, 3, -20));
  gravity.push_back(ignition::math::Vector3d(2, -3, 20));
  gravity.push_back(ignition::math::Vector3d(-2, 3, 0));
  gravity.push_back(ignition::math::Vector3d(-2, -3, -9.81));

  for (std::vector<ignition::math::Vector3d>::iterator iter = gravity.begin();
       iter != gravity.end(); ++iter)
  {
    msgs::Set(msg.mutable_gravity(), (*iter));
    physicsPub->Publish(msg);

    while (*iter != physics->Gravity())
    {
      world->Step(1);
      common::Time::MSleep(1);
    }

    EXPECT_EQ(*iter, physics->Gravity());
  }
}
...
void PhysicsMsgsTest::MoveTool(const std::string &_physicsEngine)
{
  ...
  double z0 = 5;
  ignition::math::Vector3d pos = ignition::math::Vector3d(0, 0, z0);
  ignition::math::Vector3d size = ignition::math::Vector3d(1, 1, 1);
  ...
  poses.push_back(ignition::math::Pose3d(123.456, 456.123,
        z0*10, 0.1, -0.2, 0.3));
  poses.push_back(ignition::math::Pose3d(-123.456, 456.123,
        z0*10, 0.2, 0.4, -0.6));
  poses.push_back(ignition::math::Pose3d(123.456, -456.123,
...
void PhysicsMsgsTest::LinkProperties(const std::string &_physicsEngine)
{
...

  {
    ...
    linkMsg->set_id(link->Id());
    linkMsg->set_name(link->ScopedName());

    bool newGravityMode = false;
    EXPECT_TRUE(newGravityMode != link->GravityMode());
    linkMsg->set_gravity(newGravityMode);
    modelPub->Publish(msg);

    int sleep = 0;
    int maxSleep = 50;
    while (link->GravityMode() != newGravityMode && sleep < maxSleep)
    {
      world->Step(1);
      common::Time::MSleep(100);
      sleep++;
    }
    ASSERT_TRUE(link->GravityMode() == newGravityMode);
  }
  ...
  if (_physicsEngine != "bullet" && _physicsEngine != "dart")
  {
    ...
    EXPECT_TRUE(newKinematicMode != link->Kinematic());
    linkMsg->set_kinematic(newKinematicMode);
    modelPub->Publish(msg);

    int sleep = 0;
    int maxSleep = 50;
    while (link->Kinematic() != newKinematicMode && sleep < maxSleep)
    {
      world->Step(1);
      common::Time::MSleep(100);
      sleep++;
    }
    EXPECT_TRUE(link->Kinematic() == newKinematicMode);
  }
  else
  {
...

  {
    ...
    msgs::Link *linkMsg = msg.add_link();
    linkMsg->set_id(link->Id());
    linkMsg->set_name(link->Name());

    bool newSelfCollideMode = true;
    EXPECT_TRUE(newSelfCollideMode != link->SelfCollide());
    linkMsg->set_self_collide(newSelfCollideMode);
    modelPub->Publish(msg);

    int sleep = 0;
    int maxSleep = 50;
    while (link->SelfCollide() != newSelfCollideMode && sleep < maxSleep)
    {
      world->Step(1);
      common::Time::MSleep(100);
      sleep++;
    }
    EXPECT_TRUE(link->SelfCollide() == newSelfCollideMode);
  }
}
...
void PhysicsMsgsTest::LinkPose(const std::string &_physicsEngine)
{
  ...
  {
    for (unsigned int i = 0; i < model->Links().size(); ++i)
    {
      physics::LinkPtr link = model->Links()[i];
      ASSERT_TRUE(link != NULL);
...
void PhysicsMsgsTest::SimpleShapeResize(const std::string &_physicsEngine)
{
  ...
  for (auto const &iter : modelPos)
  {
    std::string name = iter.first;
    model = world->ModelByName(name);
    sdf::ElementPtr modelElem = model->SDF();
