...
void ContactSensor::StackTest(const std::string &_physicsEngine)
{
  ...
  {
    double mass = models[k]->LinkByName()->Inertia().Mass();
    expectedForce = models[k]->LinkByName()->WorldCoGPose().Rot().Inverse()
        * ignition::math::Vector3d(0, 0, (gravityZ * mass));
    expectedTorque = ignition::math::Vector3d(0, 0, 0);

    unsigned int ColInd = 0;
    physics::CollisionPtr col = models[k]->LinkByName()->CollisionByIndex(
        ColInd);
    ...
    {
      bool body1 = true;
      if (contacts[k].contact(i).collision1() == col->ScopedName())
        body1 = true;
      else if (contacts[k].contact(i).collision2() == col->ScopedName())
        body1 = false;
      else
      {
        FAIL();
      ...
      for (int j = 0; j < contacts[k].contact(i).position_size(); ++j)
      {
        ...
        EXPECT_NEAR(contacts[k].contact(i).position(j).x(),
            models[k]->LinkByName()->WorldCoGPose().Pos().X(), TOL);
        EXPECT_NEAR(contacts[k].contact(i).position(j).y(),
            models[k]->LinkByName()->WorldCoGPose().Pos().Y(), TOL);
...
void ContactSensor::TorqueTest(const std::string &_physicsEngine)
{
  ...
  physics::CollisionPtr col =
    contactModel->LinkByName()->CollisionByIndex(ColInd);
  ASSERT_TRUE(col != NULL);
  ...
  {
    bool body1 = true;
    if (contacts.contact(i).collision1() == col->ScopedName())
      body1 = true;
    else if (contacts.contact(i).collision2() == col->ScopedName())
      body1 = false;
    else
    {
      FAIL();
    ...
    for (int j = 0; j < contacts.contact(i).position_size(); ++j)
    {
      ...
      if (_physicsEngine != "dart")
      {
        ...
        EXPECT_GT(actualTorque.Y(), fabs(actualTorque.Y()));
        EXPECT_GT(actualTorque.Z(), fabs(actualTorque.Z()));
