                             ...
                             public testing::WithParamInterface<const char*>
{
  ...
  /// Measure / verify static force torques against analytical answers.
  /// \param[in] _physicsEngine Type of physics engine to use.
  public: void ForceTorqueWithAppliedForce(
...
void JointForceTorqueTest::ForceTorque1(const std::string &_physicsEngine)
{
  ...
  physics::LinkPtr link1 = model1->LinkByName("link1");
  physics::LinkPtr link2 = model1->LinkByName("link2");
  physics::JointPtr joint01 = model1->JointByName("joint01");
  physics::JointPtr joint12 = model1->JointByName("joint12");
  ...
  for (unsigned int i = 0; i < 10; ++i)
  {
    world->Step(1);
    // test joint01 wrench
    physics::JointWrench wrench_01 = joint01->ForceTorque(0u);
    ...
    gzlog << "link1 pose [" << link1->WorldPose()
          << "] velocity [" << link1->WorldLinearVel()
          << "]\n";
    gzlog << "link2 pose [" << link2->WorldPose()
          << "] velocity [" << link2->WorldLinearVel()
          << "]\n";
    gzlog << "joint01 force torque : "
    ...
    // test joint12 wrench
    physics::JointWrench wrench12 = joint12->ForceTorque(0u);
    EXPECT_DOUBLE_EQ(wrench12.body1Force.X(),    0.0);
    EXPECT_DOUBLE_EQ(wrench12.body1Force.Y(),    0.0);
    EXPECT_FLOAT_EQ(wrench12.body1Force.Z(),  500.0);
    EXPECT_DOUBLE_EQ(wrench12.body1Torque.X(),   0.0);
    EXPECT_DOUBLE_EQ(wrench12.body1Torque.Y(),   0.0);
    EXPECT_DOUBLE_EQ(wrench12.body1Torque.Z(),   0.0);

    EXPECT_DOUBLE_EQ(wrench12.body2Force.X(),  -wrench12.body1Force.X());
    EXPECT_DOUBLE_EQ(wrench12.body2Force.Y(),  -wrench12.body1Force.Y());
    EXPECT_DOUBLE_EQ(wrench12.body2Force.Z(),  -wrench12.body1Force.Z());
    EXPECT_DOUBLE_EQ(wrench12.body2Torque.X(), -wrench12.body1Torque.X());
    EXPECT_DOUBLE_EQ(wrench12.body2Torque.Y(), -wrench12.body1Torque.Y());
    EXPECT_DOUBLE_EQ(wrench12.body2Torque.Z(), -wrench12.body1Torque.Z());
    ...
    gzlog << "link2 pose [" << link2->WorldPose()
          << "] velocity [" << link2->WorldLinearVel()
          << "]\n";
    gzlog << "joint12 force torque : "
          << "force1 [" << wrench12.body1Force
          << " / 0 0 500"
          << "] torque1 [" << wrench12.body1Torque
          << " / 0 0 0"
          << "] force2 [" << wrench12.body2Force
          << " / 0 0 -500"
          << "] torque2 [" << wrench12.body2Torque
          << " / 0 0 0"
          << "]\n";
  }
}
...
void JointForceTorqueTest::ForceTorque2(const std::string &_physicsEngine)
{
  ...
  joint01->SetParam("stop_erp", 0, 0.02);
  joint12->SetParam("stop_erp", 0, 0.02);
  ...
  for (unsigned int i = 0; i < 5; ++i)
  {
    ...
    gzlog << "joint angle1[" << std::setprecision(17) << joint01->Angle(0)
          << "] angle2[" << joint12->Angle(0) << "]\n";

    // test joint12 wrench
    physics::JointWrench wrench12 = joint12->ForceTorque(0u);
    EXPECT_NEAR(wrench12.body1Force.X(),   300.0,  3.0);
    EXPECT_NEAR(wrench12.body1Force.Y(),  -500.0,  5.0);
    if (_physicsEngine == "dart")
    {
      // DART needs greater tolerance due to joint limit violation
      // Please see issue #902
      EXPECT_NEAR(wrench12.body1Force.Z(),  -100.0,  4.3);
    }
    else
    {
      EXPECT_NEAR(wrench12.body1Force.Z(),  -100.0,  1.0);
    }
    EXPECT_NEAR(wrench12.body1Torque.X(),  250.0,  5.0);
    EXPECT_NEAR(wrench12.body1Torque.Y(),  150.0,  3.0);
    EXPECT_NEAR(wrench12.body1Torque.Z(),    0.0,  0.1);

    // A good check is that
    // the computed body1Torque shoud in fact be opposite of body1Torque
    EXPECT_NEAR(wrench12.body2Force.X(),  -wrench12.body1Force.X(),  1e-1);
    EXPECT_NEAR(wrench12.body2Force.Y(),  -wrench12.body1Force.Y(),  1e-1);
    EXPECT_NEAR(wrench12.body2Force.Z(),  -wrench12.body1Force.Z(),  1e-1);
    EXPECT_NEAR(wrench12.body2Torque.X(), -wrench12.body1Torque.X(), 1e-1);
    EXPECT_NEAR(wrench12.body2Torque.Y(), -wrench12.body1Torque.Y(), 1e-1);
    EXPECT_NEAR(wrench12.body2Torque.Z(), -wrench12.body1Torque.Z(), 1e-1);
...
/////////////////////////////////////////////////
void JointForceTorqueTest::ForceTorqueWithAppliedForce(
  const std::string &_physicsEngine)
{
  ...
  // get joint and get force torque
  physics::ModelPtr model1 = world->ModelByName("boxes");
  physics::JointPtr joint01 = model1->JointByName("joint1");
  physics::JointPtr joint12 = model1->JointByName("joint2");
  ...
  {
    // pd control
    double j1State = joint01->Angle(0u).Radian();
    double j2State = joint12->Angle(0u).Radian();
    double p1Error = target1 - j1State;
    double p2Error = target2 - j2State;
    double effort1 = kp1 * p1Error;
    double effort2 = kp2 * p2Error;
    joint01->SetForce(0u, effort1);
    joint12->SetForce(0u, effort2);
    ...
    if (i == 3387)
    {
      ...
      EXPECT_NEAR(wrench_01.body2Force.X(),
          -wrench_01.body1Force.X(),  TOL_CONT);
      EXPECT_NEAR(wrench_01.body2Force.Y(),
          -wrench_01.body1Force.Y(),  TOL_CONT);
      EXPECT_NEAR(wrench_01.body2Force.Z(),
          -wrench_01.body1Force.Z(),  TOL_CONT);
    ...
    if (i == 3387)
    {
      EXPECT_NEAR(wrench12.body1Force.X(),     0.0, TOL_CONT);
      EXPECT_NEAR(wrench12.body1Force.Y(),     0.0, TOL_CONT);
      EXPECT_NEAR(wrench12.body1Force.Z(),    50.0, TOL_CONT);
      EXPECT_NEAR(wrench12.body1Torque.X(),   25.0, TOL_CONT);
      EXPECT_NEAR(wrench12.body1Torque.Y(),    0.0, TOL_CONT);
      EXPECT_NEAR(wrench12.body1Torque.Z(),    0.0, TOL_CONT);

      EXPECT_NEAR(wrench12.body2Force.X(),   -35.355, TOL_CONT);
      EXPECT_NEAR(wrench12.body2Force.Y(),     0.000, TOL_CONT);
      EXPECT_NEAR(wrench12.body2Force.Z(),   -35.355, TOL_CONT);
      EXPECT_NEAR(wrench12.body2Torque.X(),  -17.678, TOL_CONT);
      EXPECT_NEAR(wrench12.body2Torque.Y(),    0.000, TOL_CONT);
      EXPECT_NEAR(wrench12.body2Torque.Z(),   17.678, TOL_CONT);

      gzlog << "joint12 force torque : "
            << "step [" << i
            << "] GetForce [" << joint12->Force(0u)
            << "] command [" << effort2
            << "] force1 [" << wrench12.body1Force
            << "] torque1 [" << wrench12.body1Torque
            << "] force2 [" << wrench12.body2Force
    ...
    gzlog << "angles[" << i << "] 1[" << joint01->Angle(0)
          << "] 2[" << joint12->Angle(0)
          << "]\n";
  }
}
...
void JointForceTorqueTest::JointTorqueTest(const std::string &_physicsEngine)
{
...

  {
    ...
    physics::LinkPtr link = model->LinkByName("link1");
    ASSERT_TRUE(link != NULL);
    physics::JointPtr joint = model->JointByName("joint01");
    ...
    for (unsigned int i = 0; i < 10; ++i)
    {
      ...
      world->Step(1);
      double curV = joint->Velocity(0);
      double accel = (curV - lastV) / dt;
      gzdbg << i << " : " << curV << " : " << (curV - lastV) / dt << "\n";
      lastV = curV;
      EXPECT_NEAR(accel, torque / link->Inertia().IXX(), TOL);
    }
  }

  {
    // get model
    physics::ModelPtr model = world->ModelByName("model2");
    ASSERT_TRUE(model != NULL);
    physics::LinkPtr link = model->LinkByName("link1");
    ASSERT_TRUE(link != NULL);
    physics::JointPtr joint = model->JointByName("joint01");
    ...
    for (unsigned int i = 0; i < 10; ++i)
    {
      ...
      gzdbg << i << " : " << curV << " : " << (curV - lastV) / dt << "\n";
      lastV = curV;
      EXPECT_NEAR(accel, torque / link->Inertia().IZZ(), TOL);
    }
  }
}
...
TEST_P(JointForceTorqueTest, ForceTorqueWithAppliedForce)
{
  ForceTorqueWithAppliedForce(GetParam());
}
