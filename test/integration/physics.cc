...
void PhysicsTest::SpawnDrop(const std::string &_physicsEngine)
{
  ...
  //           << "] i[" << i << "/" << steps
  //           << "] pm[" << model->WorldPose()
  //           << "] pb[" << model->LinkByName("body")->WorldPose()
  //           << "] v[" << model->WorldLinearVel()
  //           << "]\n";
...
void PhysicsTest::SpawnDropCoGOffset(const std::string &_physicsEngine)
{
  ...
  for (i = 0; i < modelNames.size(); ++i)
  {
    ...
    if (model != NULL)
    {
      ...
      ignition::math::Vector3d vel4 = model->LinkByName()->WorldLinearVel(
          ignition::math::Vector3d(0, 0, radius),
          ignition::math::Quaterniond(0, 0, 0));
...
void PhysicsTest::JointDampingTest(const std::string &_physicsEngine)
{
...

  {
    ...
    ignition::math::Vector3d vel = model->LinkByName()->WorldCoGLinearVel();
...

  {
    ...
    for (int i = 0; i < steps; ++i)
    {
      ...
      world->Step(1);  // theoretical contact, but
      {
        ...
        if (box_model)
        {
          ignition::math::Vector3d vel = box_model->WorldLinearVel();
          ignition::math::Pose3d pose = box_model->WorldPose();
          // gzdbg << "box time [" << world->SimTime().Double()
          //      << "] sim z [" << pose.Pos().Z()
          //      << "] exact z [" << z
          //      << "] sim vz [" << vel.Z()
        ...
        if (sphere_model)
        {
          ignition::math::Vector3d vel = sphere_model->WorldLinearVel();
          ignition::math::Pose3d pose = sphere_model->WorldPose();
        ...
        if (cylinder_model)
        {
          ignition::math::Vector3d vel = cylinder_model->WorldLinearVel();
          ignition::math::Pose3d pose = cylinder_model->WorldPose();
...
void PhysicsTest::InelasticCollision(const std::string &_physicsEngine)
{
...

  {
    ...
    physics::ModelPtr box_model = world->ModelByName("box");
    physics::LinkPtr box_link = box_model->LinkByName("link");
    double f = 1000.0;
    double v = 0;
    double x = 0;
    double m = box_link->Inertia().Mass();
    ...
    for (int i = 0; i < steps; ++i)
    {
      ...
      if (box_model)
      {
        ...
        if (i == 0)
        {
          ...
          // EXPECT_EQ(box_model->LinkByName("link")->WorldForce(),
          //   ignition::math::Vector3d(f, 0, 0));
        }
...
void PhysicsTest::SphereAtlasLargeError(const std::string &_physicsEngine)
{
  ...
  EXPECT_TRUE(model != NULL);
  physics::LinkPtr head = model->LinkByName("head");
...
void PhysicsTest::CollisionFiltering(const std::string &_physicsEngine)
{
  ...
  for (physics::Link_V::const_iterator iter = links.begin();
      iter != links.end(); ++iter)
  {
    std::cout << "LinkName[" << (*iter)->ScopedName() << "]\n";
