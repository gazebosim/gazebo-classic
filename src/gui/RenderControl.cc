...
void RenderControl::OnKeyDown( wxKeyEvent &event)
{
  if(event.GetKeyCode() == WXK_CONTROL)
  {
    event::Events::manipModeSignal(true);
  }
}
...
void RenderControl::Init()
{
  ...
  pose.rot.SetFromEuler( common::Vector3(DTOR(0.0),DTOR(15.0), DTOR(0.0)) ); 
...
void RenderControl::EntityRotate(Entity *entity)
{
  ...
  else
  {
    ...
    // NATY: REmove dependency on Body
...
void RenderControl::EntityTranslate(Entity *entity)
{
  ...
  common::Vector3 origin1, dir1, p1;
  common::Vector3 origin2, dir2, p2;
  ...
  common::Vector3 moveVector(0,0,0);
  common::Vector3 planeNorm(0,0,1);
