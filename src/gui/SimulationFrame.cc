  ...
  : wxFrame(parent, wxID_ANY, wxT("Gazebo"), wxDefaultPosition, wxSize(1024, 768), wxDEFAULT_FRAME_STYLE)
{
  ...
  this->connections.push_back( event::Events::ConnectAddEntitySignal( boost::bind(&SimulationFrame::AddEntityCB, this, _1) ) ); 
  this->connections.push_back( event::Events::ConnectDeleteEntitySignal( boost::bind(&SimulationFrame::DeleteEntityCB, this, _1) ) );
...
void SimulationFrame::OnToolClicked( wxCommandEvent &event )
{
  ...
  else if (id == BOX)
  {
    ...
    event::Events::createEntitySignal("box");
  }
  else if (id == SPHERE)
  {
    this->renderPanel->SetCursor(*wxCROSS_CURSOR);
    event::Events::createEntitySignal("sphere");
  }
  else if (id == CYLINDER)
  {
    this->renderPanel->SetCursor(*wxCROSS_CURSOR);
    event::Events::createEntitySignal("cylinder");
  }
  else if (id == POINT)
  {
    this->renderPanel->SetCursor(*wxCROSS_CURSOR);
    event::Events::createEntitySignal("pointlight");
  }
  else if (id == SPOT)
  {
    this->renderPanel->SetCursor(*wxCROSS_CURSOR);
    event::Events::createEntitySignal("spotlight");
  }
  else if (id == DIRECTIONAL)
  {
    this->renderPanel->SetCursor(*wxCROSS_CURSOR);
    event::Events::createEntitySignal("directionallight");
  }
  else if (id == CURSOR)
  {
    this->renderPanel->SetCursor(*wxSTANDARD_CURSOR);
    event::Events::createEntitySignal("");
  }
}
...
/* NATY: Put this back in
  if (common && common->HasType(ENTITY))
  {
    ...
    if (event.GetId() == 0)
    {
      ...
      event::Events::setSelectedEntitySignal("");
      this->toolbar->ToggleTool(CURSOR, true);
    }
    else if (event.GetId() == 1)
    {
      std::cout << "Set Selected[" << data->name << "]\n";
      event::Events::setSelectedEntitySignal(data->name);
    }
    else if (event.GetId() == 2)
    {
      event::Events::setSelectedEntitySignal("");
      event::Events::deleteEntitySignal(data->name);
    }
  }
  */
}
