...
/*  wxBoxSizer *boxSizer1 = new wxBoxSizer(wxHORIZONTAL);
  ...
  std::list<std::string> ogrePaths = GazeboConfig::Instance()->GetGazeboPaths();
...
void MeshBrowser::OnTreeClick(wxTreeEvent &event)
{
  ...
  common::Vector3 size = this->visual->GetBoundingBoxSize();
  if (size.x > 2.0 || size.y > 2.0 || size.z > 2.0)
  {
    double max = std::max(size.x,size.y);
    max = std::max(max, size.z);
    this->visual->SetScale(common::Vector3(2.0/max, 2.0/max, 2.0/max ));
  }
